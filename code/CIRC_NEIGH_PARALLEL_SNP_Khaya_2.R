...
pacman::p_load('raster', 'adegenet', 'plyr', 'stringr', 'maptools', 'parallel')
#library(raster); library(adegenet); library(raster); library(maptools); library(parallel)
...
#########################
# setwd(inputdir)
inputfile<-"code/bnut_filtered.csv"
snp_data<-read.csv(inputfile, stringsAsFactors = FALSE,h=T)
nutpoints<-snp_data[,1:4]
head(nutpoints)
a<-5
while (a<ncol(snp_data)){
  nutpoints<-cbind(nutpoints, paste(snp_data[, a], "/", snp_data[, a+1], sep="")) 
  names(nutpoints)[ncol(nutpoints)]<-names(snp_data)[a]
...
as.numeric(nutpoints$X)
as.numeric(nutpoints$Y)
r<-raster::raster(xmn=min(nutpoints$X)-2, xmx=max(nutpoints$X)+2, ymn=min(nutpoints$Y)-2, ymx=max(nutpoints$Y)+2)
...
parallel::clusterExport(cl, c("nutpoints", "r", "CIRCULARNEIGHBORHOOD", "circ.neigh", "extent", 
                    "cellFromXY", "xyFromCell", "crop", "ncell", "pointDistance"))
intermRes<-parallel::parSapply(cl, 1:nrow(nutpoints), simplify = F, FUN=function(x) circ.neigh(nutpoints, x, r, CIRCULARNEIGHBORHOOD))
stopCluster(cl)    
...
# setwd(outputdir)
write.csv(cn_trees, file=paste("output/", sp_name, "_cn_snp_data",RESOLUTION_minutes,"min_cn", CIRCULARNEIGHBORHOOD_diam_minutes,"min.csv"),row.names = F)
cn_trees<-read.csv(paste("output/", sp_name, "_cn_snp_data",RESOLUTION_minutes,"min_cn", CIRCULARNEIGHBORHOOD_diam_minutes,"min.csv"))
# Bootstrap/////////////////////////////////////////////////////////////
...
write.csv(results, file=paste("output/boot", sp_name, "_", subsample, "_plants_cn", CIRCULARNEIGHBORHOOD_diam_minutes, "min_res", RESOLUTION_minutes, "complete.csv", sep=""),row.names = F)
nas<-which(is.na(results[,5]))
results_noNA<-results[-nas,]
write.csv(results_noNA, file=paste("output/boot", sp_name, "_", subsample, "_plants_cn", CIRCULARNEIGHBORHOOD_diam_minutes, "min_res", RESOLUTION_minutes, "noNA_complete.csv", sep=""),row.names = F)
...
for (a in 4:31){
  ...
  rast<-trim(rast)
  writeRaster(rast, filename=paste("output/maps/", colnames(results)[a], "_", sp_name, "_", subsample, "_plants_cn", CIRCULARNEIGHBORHOOD_diam_minutes, "min_res", RESOLUTION_minutes, ".asc", sep=""),overwrite=T)
...
#library(maptools)
#coordinates(nutpoints)<- ~X+Y
#kmlPoints(nutpoints,kmlfile="Vigourouxpoints.kml",icon="http://google.com/mapfiles/kml/paddle/red-square-lv.png", name="")

# library(raster)
He<-raster("./output/maps/He_u_Khaya_3_plants_cn60min_res10.asc")
# library(tmap)
tm_shape(He) +
